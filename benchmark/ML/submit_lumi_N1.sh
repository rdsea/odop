...
# Run odop in a container that has access to srun
mkdir signals
rm signals/stop
export PATH="/users/rantaha1/odop_container/bin:$PATH" 
srun --overlap run_odop.py

srun singularity exec --bind /scratch/project_462000365/ container.sif\
    bash -c "RANK=\$SLURM_PROCID \
             LOCAL_RANK=\$SLURM_LOCALID \
             python3.11 GPT-neo-IMDB-finetuning.py \
		...
		--init-model-name gpt-neo-1.3B\
                ...
                --set-cpu-binds " 2>&1 > logs/output_${SLURM_JOB_NUM_NODES}.txt
