...
initialize(const Input &input, const ParallelData &parallel) {
    ...
    std::cout << "Rank " << parallel.rank << " has " << num_rows << " rows and "
              << num_cols << " columns" << std::endl;
...
void run(std::string &&fname) {
    const auto init_start_clock = MPI_Wtime();
    ...
    const auto init_stop_clock = MPI_Wtime();
    std::cout << "Initialization took " << (init_stop_clock - init_start_clock) << " seconds." << std::endl;
...
extern "C" void run(const char *fname) {
    MPI_Init(nullptr, nullptr);
    const auto start_clock = MPI_Wtime();
    heat::run(fname);
    const auto stop_clock = MPI_Wtime();
    std::cout << "Total time: " << (stop_clock - start_clock) << " seconds."
              << std::endl;
    MPI_Finalize();
}
