...
void write_field(const Field &field, const ParallelData &parallel,
    ...
    if (0 == parallel.rank) {
        const size_t buffer_size = 1024*1024*8;
        std::vector<char> buffer;
        buffer.reserve(buffer_size+1024);
        // Only the root saves the data to a png
        // save_png(data.data(), height, width, filename.c_str(), 'c');
        // Save the temperature field as a file
        std::ofstream file(filename + ".txt");
        for (size_t i = 0; i < height; i++) {
             for (size_t j = 0; j < width; j++) {
                std::string value_string = std::to_string(data[i * width + j]);
                if(j < width-1) {
                    value_string += ", ";
                }
                buffer.insert(buffer.end(), value_string.begin(), value_string.end());
