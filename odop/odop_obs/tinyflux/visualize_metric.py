...
from odop.odop_obs.core.common import SystemReport
...
def revert_unit(unit_conversion, converted_report: dict):
    ...
    for key, value in converted_report.items():
        if "unit" in key:
            if "frequency" in key:
                for original_unit, converted_unit in unit_conversion["frequency"].items():
...
for datapoint in data:
    if datapoint.tags["type"] == "process":
        converted_datapoint = {**datapoint.tags, **datapoint.fields, "timestamp": datetime.timestamp(datapoint.time)}
        converted_datapoint = unflatten(revert_unit(unit_conversion, converted_datapoint), "dot")
...
for entry in converted_data:
    timestamps.append(entry["timestamp"])  # Assuming there's a "timestamp" field in each entry
