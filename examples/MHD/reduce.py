...
def get_reduce_task(field_name):
    @odop.task(
        name=f"reduce_{field_name}",
        trigger=odop.FileUpdated(
            "/users/toukopur/pencil-code/samples/gputest/data/proc0/var.dat"
        ),
    )
...
for field_name in ["ux", "uy", "uz", "ax", "ay", "az", "lnrho", "ss"]:
...
def upload():
    ...
    with open(src_name, "rb") as file_data:
        conn.put_object(
            container, dst_name, contents=file_data, content_type="video/mp4"
        )
...
def create_video(image_folder, output_file, framerate=30):
    ...
    # TP make sure the output file always exists
    if len(images) == 0:
    ...
    video = cv2.VideoWriter(
        output_file, cv2.VideoWriter_fourcc(*"mp4v"), framerate, (width, height)
    )
    for image in images:
